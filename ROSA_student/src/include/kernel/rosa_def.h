...
enum tcbStatus_t {READY, WAITING, SUSPENDED, NONE};
typedef enum tcbStatus_t tcbStatus;
...
typedef struct tcb_record_t {
	...
	unsigned int priority;				//The task priority
	unsigned int originalPriority;
	unsigned int wakeup;				//The tasks wakeuptime
	struct tcb_record_t * previoustcb;
	struct linked_sem * taken_sem;		// All the currently locked semaphores by this task
	struct linked_sem * registered_sem;	// All semaphores this task will use
	tcbStatus status;
	void * tcbArg;
} tcb;

typedef struct linked_sem
{
	struct semHandle_s * sem;
	struct linked_sem * next_sem;
	struct linked_sem * previous_sem;
}linked_sem;
